#!/usr/bin/env python3
"""
CIFAR-100 Transient Sweep Evaluator (32×32)
==========================================

Purpose
-------
Evaluate transient diffusion samples saved at intermediate timesteps (t_*) against a
fixed REAL CIFAR-100 train set (32×32), producing per-timestep metrics + artifacts.
...
Example
-------
python3 cifar100_transient_sweep.py \
  --real_dir "/data/diffusion/repos/ddim/ddim_cifar100/datasets/cifar100/train" \
  --gen_root "/data/diffusion/repos/ddim/transient_results_cifar100_32_0.3_t0_to_t150" \
  --out_dir  "/data/diffusion/repos/ddim/transient_results_cifar100_32_0.3_t0_to_t150/eval" \
...
DEFAULT_REAL_DIR = "/data/diffusion/repos/ddim/ddim_cifar100/datasets/cifar100/train"
DEFAULT_GEN_ROOT = "/data/diffusion/repos/ddim/transient_results_cifar100_32_0.3_t0_to_t150"
DEFAULT_OUT_DIR = "/data/diffusion/repos/ddim/transient_results_cifar100_32_0.3_t0_to_t150/eval"
...
def main(cfg: SweepConfig) -> int:
    ...
    tmp_root = tempfile.mkdtemp(prefix="tmp_cifar100_transient_sweep_", dir=out_dir)

    real_tmp = None
    try:
        # ---- REAL: one-time tmp + PRDC features ----
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        "CIFAR-100 transient sweep (32×32): FID/IS (torch-fidelity) + PRDC (prdc) per timestep"
    )
    p.add_argument(
        "--real_dir",
        type=str,
        default=DEFAULT_REAL_DIR,
        help="Directory containing REAL CIFAR-100 train images (32×32).",
    )
    p.add_argument(
        "--gen_root",
        type=str,
...
if __name__ == "__main__":
    ...
    if out_dir == DEFAULT_OUT_DIR and args.gen_root != DEFAULT_GEN_ROOT:
        out_dir = os.path.join(_resolve_default_gen_root(args.gen_root), "eval")
    cfg = SweepConfig(

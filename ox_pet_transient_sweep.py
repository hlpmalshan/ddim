#!/usr/bin/env python3
"""
Oxford Pets (ox_pet) Transient Sweep Evaluator (64×64)
======================================================

Purpose
-------
Evaluate transient diffusion samples saved at intermediate timesteps (t_*) against a
fixed REAL Oxford Pets set, producing per-timestep metrics + artifacts.

Metrics (per timestep)
----------------------
- FID (torch-fidelity)
- Inception Score: mean + std (torch-fidelity)
- PRDC: precision, recall, density, coverage (prdc)

Determinism
-----------
Use a deterministic subset size n shared across ALL timesteps:
  n = min(#real, min_timestep(#gen), max_images)
and select the first n images after sorting filenames.

Real preprocessing requirement
------------------------------
REAL images are preprocessed ONCE to exactly 64×64 (resize + center crop) into a temp
folder under --out_dir, verified, and reused across all timesteps.
...
Example
-------
python3 ox_pet_transient_sweep.py \
  --real_dir "/data/diffusion/repos/ddim/ddim_ox_pet/datasets/oxford_pets/oxford-iiit-pet/images" \
  --gen_root "/data/diffusion/repos/ddim/transient_results_ox_pet_64_0.3_t0_to_t150" \
  --out_dir  "/data/diffusion/repos/ddim/transient_results_ox_pet_64_0.3_t0_to_t150/eval" \
...
DEFAULT_REAL_DIR = "/data/diffusion/repos/ddim/ddim_ox_pet/datasets/oxford_pets/oxford-iiit-pet/images"
DEFAULT_GEN_ROOT = "/data/diffusion/repos/ddim/transient_results_ox_pet_64_0.3_t0_to_t150"
DEFAULT_OUT_DIR = "/data/diffusion/repos/ddim/transient_results_ox_pet_64_0.3_t0_to_t150/eval"
...
def ensure_tmp_size(
    ...
    *,
    ...
    """
    Create a temporary directory containing one out_size×out_size RGB image per input file.

    Strategy:
    - If already RGB and out_size×out_size and not WEBP, copy as-is.
    - Otherwise, load with PIL, convert to RGB, resize+center-crop, and save as PNG.
    """
    ...
    written = 0
    for idx, src in enumerate(tqdm(files, desc=f"ensure_tmp_size[{prefix}]", leave=False)):
            ...
            with Image.open(src) as img:
                ...
                if is_rgb and is_size and ext != ".webp":
                    ...
                    if img.size != (out_size, out_size):
                        img = resize_tf(img)
                    if img.size != (out_size, out_size):
                        raise RuntimeError(f"post-resize size={img.size} (expected {(out_size, out_size)})")
...
def _verify_sample_images_exact_size(files: Sequence[str], *, expected: int, sample_n: int = 16) -> None:
    if not files:
        raise RuntimeError("No files provided for size verification.")
    k = min(int(sample_n), len(files))
    for p in files[:k]:
        with Image.open(p) as img:
            if img.size != (expected, expected):
                raise RuntimeError(f"Size check failed for {p}: got {img.size}, expected {(expected, expected)}")
...
def extract_prdc(
    ...
    *,
    ...
    This follows the repo's existing PRDC approach: flattened pixel-space tensors.
    Output shape: [N, 3*image_size*image_size] float32.
    """
    ...
    n = len(files)
    feats = np.empty((n, 3 * image_size * image_size), dtype=np.float32)
    offset = 0
...
def main(cfg: SweepConfig) -> int:
    ...
    if image_size <= 0:
        raise SystemExit(f"[ERROR] image_size must be > 0 (got {image_size})")
    ...
    tmp_root = tempfile.mkdtemp(prefix="tmp_ox_pet_transient_sweep_", dir=out_dir)

    real_tmp = None
    try:
        # ---- REAL: one-time tmp + verify size + PRDC features ----
        real_tmp, written_real = ensure_tmp_size(
            real_files, out_size=image_size, tmp_root=tmp_root, prefix=f"real{image_size}_"
        )
        if written_real < n:
            raise SystemExit(
        ...
        real_tmp_files = list_images(real_tmp)
        ...
        _verify_sample_images_exact_size(real_tmp_files, expected=image_size, sample_n=16)
        ...
        for t, d in timestep_dirs:
            ...
            gen_tmp = None
            try:
                gen_tmp, written_gen = ensure_tmp_size(
                    gen_files,
                    out_size=image_size,
                    tmp_root=tmp_root,
                    prefix=f"gen{image_size}_t{t}_",
                )
                if written_gen < n:
                    print(f"[WARN] Skip t_{t}: only wrote {written_gen}/{n} GEN images to tmp")
                    continue

                gen_tmp_files = list_images(gen_tmp)
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        "ox_pet transient sweep (64×64): FID/IS (torch-fidelity) + PRDC (prdc) per timestep"
    )
    p.add_argument(
        "--real_dir",
        type=str,
        default=DEFAULT_REAL_DIR,
        help="Directory containing REAL Oxford Pets images (will be resized to 64×64).",
    )
    p.add_argument(
        "--gen_root",
        type=str,

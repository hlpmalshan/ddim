...
BASE_CONFIG="mnist_1_3.yml"

# Experiment roots
EXP_ROOT="ddim_mnist_digits_1_3"  # adjust if digits change
DATA_ROOT="ddim_mnist_digits_1_3"  # where datasets live
...
for step in "${STEPS[@]}"; do
  ...
  for reg in "${reg_values[@]}"; do
    ...
    python "$EVAL_SCRIPT" \
      --image_path "$MNIST_IMAGE_PATH" \
      --label_path "$MNIST_LABEL_PATH" \
      --gen_dir "$GEN_DIR" \
      --digits "$DIGITS" | tee "$EVAL_TXT"
...
echo "MNIST sampling & evaluation done (checkpoint-first, no mkdir)."

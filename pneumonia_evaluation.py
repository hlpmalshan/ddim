...
def ensure_size_tmp(files, size, grayscale=False):
    ...
    for f in tqdm(files, desc=f"Prep {size}×{size}", leave=False):
        try:
            ...
            # if img.size != (size, size):
                # img = tf(img)
            # torch_fidelity expects RGB; if grayscale, convert to RGB for FID.
            if grayscale:
                img = img.convert("RGB")
...
class ImageTensorDS(Dataset):
    def __init__(self, files, size, grayscale=False):
        self.files = files
...
def main(args):
    ...
    # --- FID (torch-fidelity expects RGB) ---
    real_tmp = ensure_size_tmp(real_files, args.resolution, grayscale=args.grayscale)
    gen_tmp = ensure_size_tmp(gen_files, args.resolution, grayscale=args.grayscale)
    try:
    ...
    dl_real = DataLoader(ImageTensorDS(real_files, args.resolution, grayscale=args.grayscale), **dl_kw)
    dl_gen = DataLoader(ImageTensorDS(gen_files, args.resolution, grayscale=args.grayscale), **dl_kw)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser("Pneumonia eval @ 256: FID + PRDC (grayscale supported)")
    p.add_argument("--real_dir", required=True, type=str, help="Path to real images.")
    ...
    p.add_argument("--max_images", type=int, default=10000, help="Max images per set.")
    ...
    p.add_argument("--grayscale", action="store_true", help="Treat inputs as grayscale.")
    p.add_argument("--keep_tmp", action="store_true", help="Keep resized temp dirs.")
    args = p.parse_args()
    main(args)

#!/usr/bin/env python3
"""
Compute transient r_Theta for CIFAR-10 reverse DDPM sampling.
...
def resolve_config_path(user_config: str | None, ckpt_path: str) -> str:
    ...
    fallback_candidates = [
        Path("configs/cifar10_transient_50k.yml"),
        Path("configs/cifar10.yml"),
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Transient r_Theta for CIFAR-10 reverse DDPM sampling."
    )
    ...
    parser.add_argument("--num_images_per_t", type=int, default=1000)
    parser.add_argument(
        "--timestamps",
        type=str,
        default="auto",
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Print eps_theta and e shapes during reverse sampling.",
    )
    parser.add_argument(
        "--debug_one_iter",
        type=str2bool,
        default=False,
        help="Run only one reverse step for quick shape debugging.",
    )
        ...
        for _ in tqdm(range(n_rounds), desc="Reverse DDPM sampling for r_Theta"):
            ...
            for i, j in zip(reversed(seq), reversed(seq_next)):
                ...
                if args.debug_shapes:
                    print(
                        f"[DEBUG] reverse_step i={i} timestep_label={timestep_label} "
                        f"eps_theta.shape={tuple(eps_theta.shape)} e.shape={tuple(e.shape)}"
                    )
    ...
    if args.debug_one_iter:
        print("[INFO] debug_one_iter=True: stopped after first reverse step.")
        return 0
